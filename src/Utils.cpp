...
bool Utils::containsSuspiciousPatterns(const string& path) {
    ...
    for (size_t i = 0; i < 4; ++i) {
        ...
        // Check for other suspicious patterns - MODIFIED to allow legitimate paths
        if (currentPath.find("....") != string::npos || // Multiple dots
            currentPath.find("\0") != string::npos) { // Null byte
            return true;
        }
    }
